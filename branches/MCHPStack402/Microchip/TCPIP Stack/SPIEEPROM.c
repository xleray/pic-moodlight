...
void WriteBufferToMainMemory(XEE_ADDR address);
void ReadBufferFromMainMemory(XEE_ADDR address);
void ByteToBuffer(unsigned int byte, XEE_ADDR address);
...
unsigned char XEERead(void)
{
	...
	{ 
		...
		if (FLASHAddress > 0x01FFFF) {
		   FLASHAddress = 0x01FFFF;
		}
...
BOOL XEEIsBusy(void)
{
	...
	if(result.Val&0x80) return XEE_SUCCESS;
	else return XEE_BUSY;
