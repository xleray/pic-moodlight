...
ROM char TestAnalog[]	= "Analog test failed\0";
...
#endif
{
    ...
    while(1)
    {
	    	// Ivan - we do not blink the led here - only from the web page
    ...
    case CGI_CMD_DIGOUT:	// ACTION=0
        ...
        state_ext = TestExt();
				...
				switch(state_ext) {
					...
					case	TEST_ANALOG: memcpypgm2ram(ERRORString, TestAnalog, sizeof(TestAnalog)); break;
...
//	IEC0bits.AD1IE = 1;
	...
	// Ivan  - No program-configurable oscillator here on some boards
    OSCTUNE = 0x40;
